        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="space-y-1">
              <h3 className="text-xl font-black text-emerald-950 dark:text-white">Order #{orderId}</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={`flex items-center space-x-2 px-4 py-1.5 text-xs font-black rounded-full border uppercase tracking-widest ${
              ...
              'bg-emerald-50 dark:bg-white/5 text-emerald-700 dark:text-emerald-400 border-emerald-100'
            }`}>
              <AlertTriangle size={14} className={orderStatus === 'COMPLETED' ? 'text-emerald-500' : 'text-amber-400'} />
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-8 space-y-8">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-xl font-black text-emerald-950 dark:text-white tracking-tight">Order Items (ACTIVATE PACKAGE)</h3>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div key={item.id ?? idx} className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <h4 className="text-xl font-black text-emerald-950 dark:text-white uppercase">{item.name || item.productName || 'Order Item'}</h4>
                        ...
                        <div className="flex flex-wrap gap-2 mt-1 text-[10px] font-bold text-emerald-400">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest">{formatCurrency(item.price ?? 0)} each</p>
                      </div>
                    </div>
                    {item.store?.name && (
              ...
              {/* Package Info Box */}
              <div className="bg-white dark:bg-white/5 rounded-3xl p-6 border border-emerald-50 dark:border-white/5 space-y-4">
                <div className="flex items-center space-x-2 text-emerald-950 dark:text-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-12">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase">Daily Capping:</span>
                    <span className="text-sm font-black text-emerald-800 dark:text-emerald-200">210000</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase">Direct Commission:</span>
                    <span className="text-sm font-black text-emerald-800 dark:text-emerald-200">25%</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase">Binary Commission:</span>
                    <span className="text-sm font-black text-emerald-800 dark:text-emerald-200">12%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-indigo-50/50 dark:bg-indigo-500/5 rounded-3xl p-6 border border-indigo-100 dark:border-indigo-500/10 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-12">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase">Name:</span>
                    <span className="text-sm font-black text-emerald-800 dark:text-emerald-200">{orderItems[0]?.store?.name || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-emerald-400 uppercase">Phone:</span>
                    <span className="text-sm font-black text-emerald-800 dark:text-emerald-200">{orderItems[0]?.store?.phoneNumber || order.phoneNumber || 'N/A'}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card className="p-8 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-emerald-950 dark:text-white">{formatCurrency(totalAmount)}</span>
              </div>
              {order.orderType && (
                <div className="flex justify-between items-center">
                  <span className="text-emerald-600 font-medium">Order Type</span>
                  <span className="font-bold text-emerald-950 dark:text-white">{order.orderType}</span>
                </div>
              )}
              {order.address && (
                <div className="flex justify-between items-center">
                  <span className="text-emerald-600 font-medium">Delivery Address</span>
                  <span className="font-bold text-emerald-950 dark:text-white">{order.address}</span>
                </div>
              )}
              <div className="pt-6 border-t border-white dark:divide-white/5 flex justify-between items-center">
                <span className="text-lg font-black text-emerald-950 dark:text-white">Total</span>
                <div className="text-right">
                  <p className="text-xl font-black text-emerald-950 dark:text-white">{formatCurrency(totalAmount)}</p>
                </div>
              </div>
            </div>
          </Card>
