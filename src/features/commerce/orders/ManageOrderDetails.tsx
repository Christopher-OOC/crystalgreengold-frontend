    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 pb-12">
      ...
      <Card className="p-8">
        <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-8">
          ...
          <div className="flex flex-wrap items-center gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-3xl border border-slate-100 dark:border-white/5">
            <div className="flex flex-col space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-1.5 px-3 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-widest">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4">
              <div className="flex items-center space-x-2">
                ...
                <select 
                  ...
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg py-1 px-3 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all text-xs font-bold"
                >
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-8 space-y-8">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Order Items (BUY_PRODUCT)</h3>
            </div>

            <div className="divide-y divide-slate-100 dark:divide-white/5 space-y-8">
              {(orderItems || []).map((product: any, idx: number) => (
                <div key={product.id ?? idx} className={`flex flex-col md:flex-row gap-6 ${idx > 0 ? 'pt-8' : ''}`}>
                  <div className="w-24 h-24 bg-slate-50 dark:bg-white/5 rounded-2xl p-2 overflow-hidden border border-slate-100 dark:border-white/5 shrink-0">
                    <img 
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <h4 className="text-xl font-black text-slate-900 dark:text-white uppercase">{product.name || product.productName || 'Order item'}</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">
                          {product.categoryName || product.category || 'Uncategorized'}
                        </p>
                        <div className="flex flex-wrap gap-3 mt-1 text-[10px] font-bold text-slate-400">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">₦{product.price?.toLocaleString(undefined, { minimumFractionDigits: 2 }) ?? '0.00'} each</p>
                      </div>
                    </div>
                    {product.store?.name && (
          ...
          <Card className="p-8 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm font-medium">
                ...
                <span className="text-slate-900 dark:text-white font-bold">₦{totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              {order.orderType && (
                <div className="flex justify-between items-center text-sm font-medium">
                  <span className="text-slate-500">Order Type</span>
                  <span className="text-slate-900 dark:text-white font-bold">{order.orderType}</span>
                </div>
              )}
            </div>
          </Card>
