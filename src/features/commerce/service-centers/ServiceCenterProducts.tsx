...
interface ServiceCenterProductsProps {
  centerName: string;
  centerId: string; // This should be the member ID
  title?: string;
  onBack: () => void;
...
export const ServiceCenterProducts: React.FC<ServiceCenterProductsProps> = ({ 
  ...
  useEffect(() => {
    fetchProducts();
  }, [centerId]); // Re-fetch if centerId changes
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-bold animate-pulse tracking-widest uppercase text-xs">Loading Products...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{centerName}</h2>
