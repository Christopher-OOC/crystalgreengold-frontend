...
export const AdminUserManagement: React.FC<AdminUserManagementProps> = ({
  ...
  const getAdminRoleDisplay = (admin: AdminUser) => {
    ...
    return { text: 'Admin', icon: Shield, color: 'blue', bgColor: 'bg-blue-50', textColor: 'text-blue-500', borderColor: 'border-blue-100' };
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-slate-500 font-bold animate-pulse tracking-widest uppercase text-xs">Loading Admin Users...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4 border-l-4 border-orange-500">
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Total Admins</p>
          <p className="text-2xl font-black text-slate-900 dark:text-white">{admins.length}</p>
        </Card>
        ...
        <Card className="p-4 border-l-4 border-purple-500">
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Super Admins</p>
                ...
                <Card className="p-6 border-none shadow-xl hover:shadow-2xl transition-all group">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 font-medium mt-1">@{admin.username}</p>
                    </div>

                    <div className="pt-4 border-t border-slate-50 dark:border-white/5 space-y-3">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-black text-slate-400 uppercase tracking-widest">Role</span>
                      ...
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-bold text-slate-500 truncate max-w-[150px]">{admin.email}</span>
                      </div>
                      {admin.phoneNumber && (
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-black text-slate-400 uppercase tracking-widest">Phone</span>
                          <span className="font-bold text-slate-500">{admin.phoneNumber}</span>
                        </div>
                      )}
                      {admin.createdAt && (
                        <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="pt-4 flex items-center justify-end gap-2">
                      ...
                      <Button 
                        ...
                        className="w-10 h-10 bg-orange-500/10 text-orange-500 rounded-lg flex items-center justify-center hover:bg-orange-500 hover:text-white transition-all"
                        title="Login as user"
                      >
                        <LogIn size={16} />
                      </Button>
