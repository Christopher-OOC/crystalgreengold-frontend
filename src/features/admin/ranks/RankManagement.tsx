...
export const RankManagement: React.FC<RankManagementProps> = ({ onBack }) => {
  ...
  const stats = [
    { label: 'Total Ranks', value: ranks.length.toString(), color: 'border-blue-500' },
    ...
    { label: 'Total Qualifying BV', value: ranks.reduce((sum, r) => sum + r.qualifyingBv, 0).toLocaleString(), color: 'border-orange-500' },
  ];
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-slate-500 font-bold animate-pulse tracking-widest uppercase text-xs">Loading Rank Data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Rank Management</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Define and manage member ranking levels</p>
        </div>
      ...
      <Card noPadding className="overflow-hidden border-none shadow-2xl">
        {filteredRanks.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 font-medium">No ranks found.</p>
            <Button onClick={() => setIsCreating(true)} className="mt-4 bg-orange-600 hover:bg-orange-700 text-white">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-white/5 border-b border-slate-100 dark:border-white/5">
                  ...
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">QUALIFYING BV</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">PRIZE (₦)</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">RANK VALUE</th>
                ...
                {filteredRanks.map((rank, i) => (
                  <motion.tr key={rank.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <td className="px-6 py-5">
                      <span className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{rank.name}</span>
                    </td>
                    <td className="px-6 py-5">
                      <span className="text-sm font-bold text-slate-600 dark:text-slate-400">{rank.qualifyingBv?.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-5">
                      <span className="text-sm font-black text-slate-900 dark:text-white">₦{rank.prize?.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-5">
                      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 font-black text-sm">
                        {rank.rankValue}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end">
                        <button onClick={() => setEditingRank(rank)}
                          className="flex items-center space-x-2 text-orange-500 hover:text-orange-600 transition-colors font-black uppercase tracking-widest text-xs">
                          <Edit size={16} />
                          <span>Edit</span>
                        </button>
                      </div>
