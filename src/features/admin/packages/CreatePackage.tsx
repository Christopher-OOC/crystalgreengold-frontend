    ...
    <div className="max-w-4xl mx-auto pb-12 space-y-6 relative">
      ...
      <Card className="p-8 md:p-10 border-none shadow-2xl space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Daily Capping (₦)</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Direct Commission Rate (%)</label>
            <input type="number" name="directCommissionRate" value={form.directCommissionRate} onChange={handleChange} placeholder="0"
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all font-medium" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Binary Commission Rate (%)</label>
        ...
        <div className="space-y-4">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Package Items</label>
            ...
            {items.map((item, index) => (
              <div key={index} className="flex items-center space-x-2">
                <input type="text" placeholder="Item description" value={item} onChange={(e) => handleItemChange(index, e.target.value)}
                  className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all font-medium" />
                {items.length > 1 && (
          ...
          <button onClick={handleAddItem} className="flex items-center space-x-2 px-4 py-2 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-200 transition-colors">
            <Plus size={14} />
            <span>Add Item</span>
          </button>
        </div>
        ...
        <div className="space-y-4">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            Package Image {isEditing && <span className="text-slate-400 normal-case font-medium">(leave empty to keep current)</span>}
          </label>
          <label className="border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl p-8 flex flex-col items-center justify-center space-y-3 bg-slate-50/50 dark:bg-white/[0.02] hover:border-amber-500/50 transition-colors cursor-pointer group">
            <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
            {imagePreview ? (
              <img src={imagePreview} className="w-32 h-32 object-cover rounded-xl" />
            ) : (
              <>
                <div className="w-10 h-10 bg-slate-100 dark:bg-white/5 rounded-full flex items-center justify-center text-slate-400 group-hover:text-amber-500 transition-colors">
                  <Upload size={20} />
                </div>
                <div className="text-center space-y-1">
                  <p className="text-xs font-bold text-slate-600 dark:text-slate-400">Click to upload or drag and drop</p>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">PNG, JPG, GIF (MAX. 5MB)</p>
                </div>
              </>
            )}
          </label>
