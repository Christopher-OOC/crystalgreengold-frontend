    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-0.5">
          <button 
            onClick={onBack}
            className="flex items-center space-x-2 text-emerald-600 hover:text-amber-400 transition-colors font-bold text-xs group mb-1"
          >
            <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" />
            <span>Back to Dashboard</span>
          </button>
          <h2 className="text-xl font-black text-emerald-950 dark:text-white tracking-tight uppercase">My Profile</h2>
          <p className="text-emerald-600 font-medium text-xs">
            Manage your personal information and account security
          </p>
        </div>

        <div className="flex items-center p-1 bg-emerald-50 dark:bg-white/5 rounded-xl border border-emerald-100 dark:border-white/5">
          {tabs.map(tab => (
            <button
              key={tab.id}
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                activeSubTab === tab.id 
                  ? 'bg-white dark:bg-emerald-900 text-amber-400 shadow-md' 
                  : 'text-emerald-600 hover:text-emerald-950 dark:hover:text-white'
              }`}
            >
              <tab.icon size={12} />
              ...
              <div className="px-6 -mt-14 relative z-10">
                <div className="flex flex-col md:flex-row items-end gap-4">
                  <div className="relative group">
                    <div className="w-28 h-28 rounded-[24px] bg-white dark:bg-emerald-950 p-1.5 shadow-xl border border-white/10">
                    ...
                    <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-emerald-500 rounded-xl flex items-center justify-center border-2 border-white dark:border-emerald-950 shadow-lg">
                  ...
                  <div className="flex-1 pb-2 space-y-1">
                    <div className="flex items-center space-x-2">
                      <h1 className="text-2xl font-black text-emerald-950 dark:text-white tracking-tight">
                        {safeMember.firstName} {safeMember.lastName}
                      </h1>
                      <span className="px-2 py-0.5 bg-amber-400/10 text-amber-400 text-[9px] font-black rounded-md uppercase tracking-widest border border-amber-400/20">
                        Pro Member
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-3 text-emerald-600 dark:text-emerald-400 font-medium text-xs">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="space-y-6">
                <Card className="p-4 space-y-4">
                  <h3 className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">Quick Stats</h3>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-white/5 rounded-xl">
                        <div className="flex items-center space-x-2">
                          <div className="p-1.5 rounded-lg bg-amber-400/10 text-amber-400">
                            <stat.icon size={16} />
                          </div>
                          <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400">{stat.label}</span>
                        </div>
                        <span className="text-base font-black text-emerald-950 dark:text-white">{stat.value}</span>
                      </div>
                    ))}
                  </div>
                </Card>
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card className="p-5 space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-base font-black text-emerald-950 dark:text-white uppercase tracking-tight">Account Overview</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div className="space-y-0.5">
                        <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Full Name</p>
                        <p className="text-base font-bold text-emerald-950 dark:text-white">{safeMember.firstName} {safeMember.lastName}</p>
                      </div>
                      <div className="space-y-0.5">
                        <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Email Address</p>
                        <p className="text-base font-bold text-emerald-950 dark:text-white">{safeMember.email}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div className="space-y-0.5">
                        <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Username</p>
                        <p className="text-base font-bold text-emerald-950 dark:text-white">@{safeMember.username}</p>
                      </div>
                      <div className="space-y-0.5">
                        <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Membership Type</p>
                        <p className="text-base font-bold text-emerald-950 dark:text-white">{safeMember.memberType}</p>
                      </div>
                    </div>
                  </div>
                </Card>
